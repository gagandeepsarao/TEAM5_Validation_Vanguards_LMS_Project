package constants;

public class Constants {
	
	public static String BROWSER;
    public static String USER;
    public static String PASSWORD;
    public static String URL;
    public static String EXCELPATH;
    public static String INVALIDURL;
    public static String DASHBOARDURL;
    public static String INCORRECTUSERNAME;
    public static String INCORRECTPASSWORD;
    public static String DashboardPage;
    public static String userPageURL;
    // user page
    public static String TableHeaderColumn1;	// ID
    public static String TableHeaderColumn2;	// Name
    public static String TableHeaderColumn3;	// Location	
    public static String TableHeaderColumn4;	// Phone Number
    public static String TableHeaderColumn5;	// Edit/Delete

    
}
